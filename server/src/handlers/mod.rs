...
pub async fn stats(State(state): State<AppState>, headers: axum::http::HeaderMap) -> impl IntoResponse {
    ...
    match state.db.get().await {
        Ok(client) => {
            let total_cards = client.query_opt(
                "SELECT count(*)::int FROM flashcards WHERE user_id = $1", &[&user_id],
            ).await.ok().and_then(|r| r.map(|r| r.get::<_, i32>(0))).unwrap_or(0);

            let total_reviews = client.query_opt(
                "SELECT count(*)::int FROM study_sessions WHERE user_id = $1", &[&user_id],
            ).await.ok().and_then(|r| r.map(|r| r.get::<_, i32>(0))).unwrap_or(0);

            let mastered = client.query_opt(
                "SELECT count(*)::int FROM srs_reviews WHERE user_id = $1 AND leitner_box >= 4", &[&user_id],
            ).await.ok().and_then(|r| r.map(|r| r.get::<_, i32>(0))).unwrap_or(0);
